package me.alpha432.oyvey.features.modules.combat.burrow;
...
import me.alpha432.oyvey.features.modules.combat.burrow.utils.Timer;
import me.alpha432.oyvey.event.impl.autototem.EventPostSync;
import me.alpha432.oyvey.features.modules.combat.burrow.utils.player.*;
import me.alpha432.oyvey.features.modules.combat.burrow.utils.MathUtility;
...
public final class Burrow extends Module {
    private final Setting<Mode> mode = this.register(new Setting<>("Mode", Mode.Default));
    private final Setting<OffsetMode> offsetMode = this.register(new Setting<>("Mode", OffsetMode.Smart, v -> mode.getValue() == Mode.Default));
    public Setting<Float> vClip = this.register(new Setting<>("V-Clip", -9.0F, -256.0F, 256.0F, v -> offsetMode.getValue() == OffsetMode.Constant && mode.getValue() == Mode.Default));
    public Setting<Integer> delay = this.register(new Setting<>("Delay", 100, 0, 1000, v -> mode.getValue() == Mode.Default));
    public Setting<Boolean> scaleDown = this.register(new Setting<>("Scale-Down", false, v -> mode.getValue() == Mode.Default));
    public Setting<Boolean> scaleVelocity = this.register(new Setting<>("Scale-Velocity", false, v -> mode.getValue() == Mode.Default));
    public Setting<Boolean> scaleExplosion = this.register(new Setting<>("Scale-Explosion", false, v -> mode.getValue() == Mode.Default));
    public Setting<Float> scaleFactor = this.register(new Setting<>("Scale-Factor", 1.0F, 0.1F, 10.0F, v -> mode.getValue() == Mode.Default));
    public Setting<Integer> scaleDelay = this.register(new Setting<>("Scale-Delay", 250, 0, 1000, v -> mode.getValue() == Mode.Default));
    public Setting<Boolean> attack = this.register(new Setting<>("Attack", true, v -> mode.getValue() == Mode.Default));
    public Setting<Boolean> placeDisable = this.register(new Setting<>("PlaceDisable", false, v -> mode.getValue() == Mode.Default));
    public Setting<Boolean> wait = this.register(new Setting<>("Wait", true));
    public Setting<Boolean> evade = this.register(new Setting<>("Evade", false, v -> offsetMode.getValue() == OffsetMode.Constant && mode.getValue() == Mode.Default));
    public Setting<Boolean> noVoid = this.register(new Setting<>("NoVoid", false, v -> offsetMode.getValue() == OffsetMode.Smart && mode.getValue() == Mode.Default));
    public Setting<Boolean> onGround = this.register(new Setting<>("OnGround", true, v -> mode.getValue() == Mode.Default));
    public Setting<Boolean> allowUp = this.register(new Setting<>("IgnoreHeadBlock", false, v -> mode.getValue() == Mode.Default));
    public Setting<Boolean> rotate = this.register(new Setting<>("Rotate", true));
    public Setting<Boolean> discrete = this.register(new Setting<>("Discrete", true, v -> offsetMode.getValue() == OffsetMode.Smart && mode.getValue() == Mode.Default));
    public Setting<Boolean> air = this.register(new Setting<>("Air", false, v -> offsetMode.getValue() == OffsetMode.Smart && mode.getValue() == Mode.Default));
    public Setting<Boolean> fallback = this.register(new Setting<>("Fallback", true, v -> offsetMode.getValue() == OffsetMode.Smart && mode.getValue() == Mode.Default));
    public Setting<Boolean> skipZero = this.register(new Setting<>("SkipZero", true, v -> offsetMode.getValue() == OffsetMode.Smart && mode.getValue() == Mode.Default));
    ...
    public Burrow() {
        super("Burrow", "Burrow", Module.Category.COMBAT, true, false, false);
        instance  = this;
    }
    ...
    public void onPostSync(EventPostSync e) {
        if (wait.getValue()) {
            ...
            if (!currentPos.equals(startPos)) {
                disable("Disabled due to movement!");
                return;
            }
        }
        ...
        if (!mc.world.getBlockState(pos).isReplaceable()) {
            if (!wait.getValue())
            disable("Can't place the block! Disabling..");
            return;
        }
        ...
        for (Entity entity : mc.world.getNonSpectatingEntities(Entity.class, new Box(pos))) {
            if (entity != null && !mc.player.equals(entity)) {
                ...
                if (!wait.getValue())
                disable("Can't place the block on! Disabling..");
                return;
            }
        }
    ...
    private void handleWeb(BlockPos pos) {
        ...
        if (timer.passedMs(1000)) {
            ...
            disable("Successfully burrowed! Disabling..");
        }
    }
    ...
    public void handleDefault(BlockPos pos) {
        ...
        if (!allowUp.getValue()) {
            ...
            if (upState.blocksMovement()) {
                if (!wait.getValue())
                disable("Above the head block, impossible to burrow! Disabling..");
                return;
            }
        }
        ...
        if (mc.isInSingleplayer()) {
            disable("Retard! You're in singleplayer..");
            return;
        }
