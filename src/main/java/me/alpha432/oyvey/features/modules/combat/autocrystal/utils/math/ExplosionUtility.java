...
public final class ExplosionUtility {
    ...
    public static float getExplosionDamage2(Vec3d crystalPos, PlayerEntity target) {
        try {
            if (AutoCrystal.getInstance().predictTicks.getValue() == 0) return getExplosionDamage1(crystalPos, target);
            return getExplosionDamageWPredict(crystalPos, target, PredictUtility.predictPlayer(target, AutoCrystal.getInstance().predictTicks.getValue()));
