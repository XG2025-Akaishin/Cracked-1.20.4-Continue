...
public class MiddleClick extends Module {
    ...
    private final Setting<Boolean> ep = this.register(new Setting<>("Pearl", true));
    private final Setting<Boolean> silentPearl = this.register(new Setting<>("SilentPearl", true, v -> ep.getValue()));
    private final Setting<Boolean> inventoryPearl = this.register(new Setting<>("InventoryPearl", true, v -> ep.getValue()));
    ...
    private final Setting<Boolean> noWasteXp = this.register(new Setting<>("Anti Waste", true, v -> xp.getValue()));
    public final Setting<Boolean> antiPickUp = this.register(new Setting<>("AntiPickUp", true));
    private final Setting<Integer> durability = this.register(new Setting<>("Stop On", 90, v -> xp.getValue()));
    ...
    private void onSync(EventSync event) {
        ...
        if (friend.getValue() && mc.currentScreen == null && mc.options.pickItemKey.isPressed() && mc.targetedEntity != null && mc.targetedEntity instanceof PlayerEntity entity && timer.passedMs(800)) {
            if (OyVey.friendManager.isFriend(entity.getName().getString())) {
                OyVey.friendManager.removeFriend(entity.getName().getString());
                sendMessage("§b" + entity.getName().getString() + "§r removed from friends!");
            } else {
                OyVey.friendManager.addFriend(entity.getName().getString());
