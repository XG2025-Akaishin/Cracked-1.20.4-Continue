package me.alpha432.oyvey.features.modules.combat.autoarmor;
...
import me.alpha432.oyvey.features.modules.combat.autoarmor.utils.Timer;


public class AutoArmor extends Module {

    public Setting<Float> delay = this.register(new Setting<>("Delay", 50.0f, 0.0f, 1000.0f));//The amount of delay in milliseconds
    public Setting<Boolean> curse = this.register(new Setting<>("Curse", false));
    public Setting<Boolean> PreferElytra = this.register(new Setting<>("Elytra", false));
    public Setting<Boolean> ElytraReplace = this.register(new Setting<>("ElytraReplace", false));

    private Timer timer = new Timer();

    public AutoArmor() {
        super("AutoArmor", "AutoArmor ou", Category.COMBAT, true, false, false);
    }
    ...
    public void OnPlayerUpdate(PlayerMotionUpdate event) {
        ...
        if (ElytraReplace.getValue() && !mc.player.getInventory().getStack(38).isEmpty()) {
            ItemStack stack = mc.player.getInventory().getStack(38);

            if (stack.getItem() instanceof ElytraItem) {
                if (!ElytraItem.isUsable(stack) && GetPctFromStack(stack) < 3) {
                    for (int i = 0; i < 36; i++) {
    ...
    private int findArmorSlot(EquipmentSlot type) {
        ...
        for (int i = 0; i < 36; i++) {
            ...
            if (s != null && s.getItem() != Items.AIR) {
                ...
                } else if (type == EquipmentSlot.CHEST && PreferElytra.getValue() && s.getItem() instanceof ElytraItem && GetPctFromStack(s) > 3)
                    return (i < 9 ? i + 36 : i);
            }
        }
